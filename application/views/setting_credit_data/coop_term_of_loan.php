				...
				<form id='form_save' data-toggle="validator" novalidate="novalidate" action="<?php echo base_url(PROJECTPATH . '/setting_credit_data/coop_term_of_loan_save'); ?>" method="post">
					...
					<div class="row">
						<div class="form-group">
							...
							<div class="col-sm-4">
								<input name="contract_pattern" id="contract_pattern" class="form-control m-b-1" type="text" value="<?php echo @$row['contract_pattern']; ?>" required>
							</div>
							<label class="col-sm-4 control-label_2"> <a class="" href="#" onclick="contract_pattern_help()">วิธีการตั้งค่า <i class="fa fa-info" style="font-size:20px"></i></a></label>
						</div>
					</div>
							...
							<tbody>
								...
								?>
										<tr>
											...
											<td>
												<a style="cursor:pointer;" onclick="edit_loan_name('<?php echo @$value['loan_name_id']; ?>','<?php echo @$value['loan_type_id']; ?>','<?php echo @$value['loan_name']; ?>','<?php echo @$value['loan_name_description']; ?>','<?php echo @$value['loan_name_status']; ?>','<?php echo @$value['loan_app']; ?>');">แก้ไข</a>
												|
