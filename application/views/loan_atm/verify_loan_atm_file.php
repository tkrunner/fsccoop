    ...
    <div class="layout-content-body">
        ...
        <div class="row">
            <div class="form-group">
                <div class="col-sm-6">
                    <h1 class="title_top">ตรวจสอบข้อมูลไฟล์ทำรายการ ATM</h1>
        ...
        <div class="row gutter-xs">
            <div class="col-xs-12 col-md-12">
                <div class="panel panel-body" style="padding-top:15px !important;">
                    <h3>อัพโหลดไฟล์ตรวจสอบข้อมูลทำรายการ ATM</h3>
                    <div class="g24-col-sm-24" style="text-align:right;padding-right:0px;margin-right:0px;">
                        <form class="form-group" id="atm_file_upload" name="atm_file_upload" method="post" enctype="multipart/form-data" action="<?php echo base_url('/loan_atm/upload_check_file'); ?>">
                            <label class="g24-col-sm-6 control-label ">ไฟล์ตรวจสอบข้อมูลทำรายการ ATM :</label>
                            <div class="g24-col-sm-6">
                                <span id="filename">ยังไม่ได้เลือกไฟล์</span>
                                <label class="fileContainer btn btn-info">
                                    <span class="icon icon-paperclip"></span>
                                    เลือกไฟล์
                                    <input type="file" class="form-control" id="file_attach" name="file" value="" multiple>
                                </label>
                            </div>
                        ...
                        <div class="g24-col-sm-9 content-process">
                            <div class="progress">
                                <div class="bar"></div>
                                    ...
                                    <?php if(!empty($file['file_name'])){ ?>
                                        ...
                                        <td>
                                            <?php if(!empty($file['id'])){?>
                                                <button class="btn btn-circle btn-primary" <?php echo !empty($file['active_status']) ? '' : 'disabled="disabled"' ?> onclick="process(<?php echo $file['id']; ?>)" title="อ่านไฟล์ข้อมูล">
                                                    <i class="fa fa-eye" aria-hidden="true"></i>
                                                </button>
                                            <?php } ?>
                                            <?php if(!empty($file['id'])){?>
                                                <button class="btn btn-circle btn-primary" <?php echo $file['status'] == 0 && $file['active_status'] == 0 ? '' : 'disabled="disabled"' ?> onclick="save(<?php echo $file['id']; ?>)" title="บันทึกข้อมูล">
                                                    <i class="fa fa-save" aria-hidden="true"></i>
                                                </button>
                                            <?php }?>
                                            <?php if(!empty($file['id'])){?>
                                                <button class="btn btn-circle btn-primary" <?php echo $file['status'] == 0 && $file['active_status'] == 0 ? '' : 'disabled="disabled"' ?> onclick="delete_file(<?php echo $file['id']; ?>)" title="ลบไฟล์">
...
<div class="modal fade" id="modal_verify_check_list" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header modal-header-info">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h3 class="modal-title">ไฟล์ตรวจสอบข้อมูลการผูกบัญชี ATM</h3>
            </div>
...
$v = date('YmdHis');
$link = array(
    'src' => PROJECTJSPATH . 'assets/js/verify_loan_atm_file.js?v=' . $v,
