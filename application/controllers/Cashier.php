    ...
    public function index() {
        ...
        if ($member_id != '') {
            ...
            foreach ($row as $key => $value) {
                ...
                $row[$key]['interest'] = round($interest_loan, 2);
                ...
                if($loan_balance_setting == 1 && !empty($value['finance_month_pay_amount'])) {
                    $row[$key]['loan_amount_balance'] = $value['loan_amount_balance'] - $value['finance_month_pay_amount'];
                }
            }
			...
			$this->db->select(array(
				't1.loan_atm_id',
                't1.contract_number',
                't2.pay_amount as finance_month_pay_amount'
			));
            $this->db->from('coop_loan_atm as t1');
            $this->db->join('coop_finance_month_detail as t2', 't1.loan_atm_id = t2.loan_atm_id AND t2.run_status = 0 AND t2.pay_type = "principal"', "left");

			$this->db->where("
				t1.member_id = '".$member_id."' 
				AND t1.loan_atm_status = '1' 
				AND t1.total_amount_approve != t1.total_amount_balance
			");
			$row = $this->db->get()->result_array();
			foreach($row as $key => $value){
				...
				$cal_atm_interest['date_interesting'] = $date_now;
				...
				@$row[$key]['principal'] = @$principal;
				$row[$key]['interest'] = number_format($interest,2, '.', '');
				//Get loan interest non pay
				...
				$loan_interest_debt = $this->db->select("loan_atm_id, SUM(interest_balance) AS sum_interest_balance")
												->from("coop_loan_interest_debt")
												->where("loan_atm_id = '".$value['loan_atm_id']."' AND interest_status = 0  AND member_id = '".$member_id."'")
												->get()->row();
				if(!empty($loan_interest_debt)) {
					$loan_interest_debt_total = @$loan_interest_debt->sum_interest_balance;
				}		
				$row[$key]['loan_interest_debt_total'] = number_format($loan_interest_debt_total,2, '.', '');
				$row[$key]['interest_non_pay'] = number_format(@$loan_interest_remain_total+@$loan_interest_debt_total,2, '.', '');
			}
    ...
    function save(){
        ...
        foreach ($data_post['account_list'] as $key => $value) {
            ...
            if ($data_post['loan_id'][$key] != '') {
                ...
                $atm_transaction['interest_from'] = $row_loan_atm['date_last_interest'];
                $atm_transaction['interest_to'] = $date;
                $atm_transaction['loan_type_code'] = 'PM';
                $atm_transaction['interest_arrears'] = $data_post['interest'][$key];
                $atm_transaction['interest_arrear_bal'] = 0;
                $atm_transaction['interest_notpay'] = 0;
