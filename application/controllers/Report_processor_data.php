...
class Report_processor_data extends CI_Controller {
	...
	function coop_report_charged_department_preview(){
		...
		$arr_data = array();
        $code_name = 'coop_report_charged_department_preview';

        $this->db->select('*');
        $this->db->from('format_setting_report');
        $this->db->where('code_name', $code_name);
        $switch_report = $this->db->get()->row_array();
		$this->load->model('report_processor_data_model');
        if($switch_report['switch_code'] == '1') {
            $arr_data = $this->report_processor_data_model->get_coop_report_charged_department_preview();
        }else if($switch_report['switch_code'] == '2'){
			$arr_data = $this->report_processor_data_model->get_coop_report_charged_department_preview_2();
		}
        ...
        if($switch_report['switch_code'] == '1') {
            if($_GET['type_department'] == '1'){
				...
				$arr_data = $this->report_processor_data_model->get_coop_report_charged_person_preview($arr_data);
				$this->preview_libraries->template_preview('report_processor_data/coop_report_charged_person_preview',$arr_data);
			}else{
			...
			if($_GET['type_department'] == '1'){
				$this->preview_libraries->template_preview('report_processor_data/coop_report_charged_department_preview_2',$arr_data);
			} else if ($_GET['type_department'] == '3') {
				// $this->preview_libraries->template_preview('report_processor_data/coop_report_charged_person_preview',$arr_data);
				$arr_data = $this->report_processor_data_model->get_coop_report_charged_person_preview_2($arr_data);
				$this->preview_libraries->template_preview('report_processor_data/coop_report_charged_person_preview_2',$arr_data);
			}else{
				$this->preview_libraries->template_preview('report_processor_data/coop_report_charged_level_preview_2',$arr_data);
			}
		}
	...
	function coop_report_charged_person_excel() {
		...
		$this->load->model('report_processor_data_model');
        if($switch_report['switch_code'] == '1') {
			$arr_data = $this->report_processor_data_model->get_coop_report_charged_person_excel();
			$this->load->view('report_processor_data/coop_report_charged_person_excel',$arr_data);
        }else if($switch_report['switch_code'] == '2'){
			$arr_data = $this->report_processor_data_model->get_coop_report_charged_person_excel_2();
			$this->load->view('report_processor_data/coop_report_charged_person_excel_2',$arr_data);
		}
	}
