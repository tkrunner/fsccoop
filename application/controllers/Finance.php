			...
			foreach($deduct_list as $key2 => $value2){
				...
				if($value2['deduct_code']=='OTHER'){
							...
							if($value3['pay_loan_type'] == '2' && $value3['pay_type'] == 'principal'){
								...
								if(($value3['loan_amount_balance']) >= @$value3['money_period_1']){
									if($value3['check_month'] == 0){
										...
										// var_dump($query_interest);
										if($query_interest['pay_type']==1){ //จ่ายแบบคงต้น
											$data_arr[$p]['pay_amount'] = round($query_interest['money_period'], $this->Setting_model->get("round_interest"));
										}else{//จ่ายแบบคงยอด
											$in = round($query_interest['interest'], $this->Setting_model->get("round_interest"));
								...
								if($value3['check_month'] == 0 && $value3['pay_type'] != 'principal'){
                                    ...
                                    $flag = "";
                                    if(!$this->Interest_modal->first_interest_payment($value3['loan_id'])){
                                        $flag = "+".$this->Setting_model->get("first_interest_payment");
                                    }
									$sql = 'SELECT
													id as loan_id,
													DATEDIFF(
									...
									// echo "<hr>";
									// exit;
									$data_arr[$p]['pay_amount'] = round($query_interest['interest'], $this->Setting_model->get("round_interest"));
					...
					*/
					...
					$last_month = Date("Y-m-d", strtotime($process_month." -0 Month"));
					...
					if($value_process['real_pay_amount']>0){
						...
						if($value_process['deduct_code'] == 'LOAN' || $value_process['deduct_code'] == 'GUARANTEE'){
							...
							if($value_process['pay_type'] == 'principal'){
								...
								$pay_amount = '';
								$balance = '';
								$transaction_text = $this->Finance_libraries->generate_loan_receipt_text_finance_month($row_loan['loan_id'], 15, "interest");
							}
