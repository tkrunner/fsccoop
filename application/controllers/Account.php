...
class Account extends CI_Controller {
    ...
    public function account_save() {
        ...
        if(!empty($_POST["account_id"])) {
            ...
            $data_insert['user_id'] = $_SESSION['USER_ID'];
            $data_insert['edit_at'] = $_SESSION['USER_ID'];
            ...
            foreach($account_details as $detail) {
                $this->account_transaction->increase_decrease_budget_year($detail["account_chart_id"], $detail["account_amount"], $detail["account_type"], $budget_year_be, 2);
            }
    ...
    function account_chart_report_excel() {
        ...
        foreach($row['data']  as $key => $row) {
            ...
            foreach($row as $key2 => $row_detail){
                ...
                $data_account_detail[$account_datetime[0]][$row_detail['account_id']][$row_detail['account_chart_id'].$row_detail['account_type']]['account_description'] = $row_detail['account_description'];
    ...
    function get_account_balance_data($data) {
        ...
        $perv_account_raw = $this->db->select("t2.account_chart_id, t2.account_type, t2.account_amount, t3.entry_type")
                                        ->from("coop_account as t1")
                                        ->join("coop_account_detail as t2", "t1.account_id = t2.account_id AND t2.account_chart_id != '30005'", "INNER")
                                        ->join("coop_account_chart as t3", "t2.account_chart_id = t3.account_chart_id", "LEFT")
    ...
    public function account_pdf_tranction_voucher() {
        ...
        if($voucher_type == 1) {
            if(!empty($_POST['account_detail_ids'])) {
                $accounts = $this->db->select("journal_ref,
                                                journal_type,
                                                account_datetime,
                                                account_description,
                                                account_id")
                                    ->from("coop_account")
                                    ->where("account_id in (".implode(',',$account_detail_ids).")")
                                    ->get()->result_array();
                $account_details = array();
    ...
    public function cancel_account_transaction() {
        ...
        $accounts = $this->db->select("t1.budget_year, t2.account_type, t2.account_amount, t2.account_chart_id")
                            ->from("coop_account as t1")
                            ->join("coop_account_detail as t2", "t1.account_id = t2.account_id", "inner")
                            ->where("t1.account_id = '".$_POST["account_id"]."'")
                            ->get()->result_array();
