...
class Contract_modal extends CI_Model
{
    ...
    function getPrevLoan($member_id, $createdatetime){
        ...
        foreach($prev_loan_active as $key => $value){
            ...
            if(sizeof($row)) {
                foreach ($row as $key2 => $value2) {
                    if($value2['pay_type'] == 'interest' && date('t', strtotime($date_interesting)) == date('j', strtotime($date_interesting))){
                        $fin_interest += $value2['pay_amount'];
                    }else if($value2['pay_type'] == 'principal'){
                ...
                if($this->Setting_model->get("calc_interest_on_getPrevLoan_have_finance_month_detail")==1){
                    ...
                    $interest_loan = ROUND($interest_loan-$fin_interest, 2);
                }
            }else{
                $interest_loan = $this->loan_libraries->calc_interest_loan_multi_rate($loan_amount, $loan_type, $date1, $date2);
            ...
            $prev_loan_active_arr[$i]['sql'] = $sql;
        ...
        foreach($row_prev_loan as $key => $value){
            ...
            $atm_data['date_interesting'] = $date_interesting;
            ...
            $prev_loan_active_arr[$i]['prev_loan_total'] = $loan_balance;
