<?php
    ...
    if ( strtolower($platform) == 'ios' ) {
        ...
        if ( $review_appstore AND $version == '1.2' AND $PIN == '998877' ) {
    ...
    //             $member_arr = '';
    //             if ( strlen(trim($arr_member[$i])) == 1 ) {
    //                 $member_arr = '00000'.trim($arr_member[$i]);
    //             } else if ( strlen(trim($arr_member[$i])) == 2 ) {
    //                 $member_arr = '0000'.trim($arr_member[$i]);
    //             } else if ( strlen(trim($arr_member[$i])) == 3 ) {
    //                 $member_arr = '000'.trim($arr_member[$i]);
    //             } else if ( strlen(trim($arr_member[$i])) == 4 ) {
    //                 $member_arr = '00'.trim($arr_member[$i]);
    //             } else if ( strlen(trim($arr_member[$i])) == 5 ) {
    //                 $member_arr = '0'.trim($arr_member[$i]);
    //             } else if ( strlen(trim($arr_member[$i])) == 6 ) {
    //                 $member_arr = trim($arr_member[$i]);
    //             }
    ...
    if ( strtolower($loginVia) == "pin" ) {
        $sql = "SELECT tb1.member_id, tb1.mobile_PIN, tb2.member_status,tb2.mem_type
        ...
        if ( $rs->num_rows ) {
            ...
            if ( $row['mobile_PIN'] == $encrypt_PIN ) {
                ...
                $data['PIN'] = $row['mobile_PIN'];
                $data['REPIN'] = $encrypt_PIN;
                $data['REPINOR'] = $PIN;
                
            }
        } else {
            if($PIN == '998877' AND  strtolower($platform) == 'ios'){
                $member_id = '001316';
