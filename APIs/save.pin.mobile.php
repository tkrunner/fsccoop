<?php 
    ...
    //$sql = "SELECT mobile_id FROM mobile_token WHERE trim(member_id) = trim('{$member_id}') AND trim(mobile_uid) = trim('{$mobile_uid}') AND is_delete = 0 ORDER BY create_date DESC";
    $sql = "SELECT mobile_id FROM mobile_token WHERE mobile_id = {$mobile_tokenId} AND is_delete = 0";
    $rs = $mysqli->query($sql);
    if ( $rs->num_rows ) {
        $row = $rs->fetch_assoc();
