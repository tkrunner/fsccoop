<?php
    ...
    // $member_id = '999999';
    $contract_number = isset($_POST['contract_number']) ? $mysqli->real_escape_string($_POST['contract_number']) : null;
    $type_send = isset($_POST['type']) ? $mysqli->real_escape_string($_POST['type']) : null;
    $data['data']['loan'] = [];
    if($type_send == 'loan'){
        $sql = "SELECT 
        tb1.id, 
        ...
        $sql = "SELECT loan_atm_id,FORMAT(total_amount, 2) as  total_amount ,approve_date,contract_number,FORMAT(total_amount_balance, 2) as total_amount_balance ,max_period,loan_atm_status
    ...
    //     $sql = "SELECT loan_atm_id, FORMAT(total_amount, 2) as  total_amount ,approve_date,contract_number,FORMAT(total_amount_balance, 2) as total_amount_balance ,max_period,loan_atm_status
    //     ,createdatetime FROM coop_loan_atm AS tb1
    //     WHERE trim(member_id) = trim('{$member_id}') AND trim(loan_atm_status) = '1'
    //     ORDER BY createdatetime DESC";
    // }
