<?php
    ...
    $loan_type = isset($_POST['loan_type']) ? $mysqli->real_escape_string($_POST['loan_type']) : null;
    $contract_number = isset($_POST['contract']) ? $mysqli->real_escape_string($_POST['contract']) : null;
    ...
    // OR tb1.member_id = '{$member_id}' AND loan_status = '6'
    ...
    //     }
    // }
    if($loan_type != "เงินกู้ฉุกเฉิน ATM"){
        $sql = "SELECT 
    tb1.id, 
    tb1.approve_date,
    IF(tb1.contract_number = '', 'N/A', tb1.contract_number) AS contract_number, 
    ...
    ORDER BY createdatetime DESC";
     ...
     if ( $rs->num_rows ) {
        ...
        tb1.loan_atm_id,tb4.contract_number,tb4.approve_date,tb4.total_amount_approve,tb4.total_amount_balance,tb4.loan_atm_status, 
        ...
        LEFT JOIN coop_loan_atm as tb4 ON tb1.loan_atm_id = tb4.loan_atm_id
...
if ( $rs->num_rows ) {
    while( ($row = $rs->fetch_assoc()) ){
        $data['data'] = [
            ...
            'approve_date' => ($row['approve_date'] == null) ? 'N/A' : dateDB2thaidate($row['approve_date'],true,false,false),
            'contract_number' => $row['contract_number'],
            'loan_amount' => number_format($row["total_amount_approve"] , 2 , "." , ",") ,
            'loan_amount_balance' =>  number_format($row["total_amount_balance"] , 2 , "." , ","),
            'period_amount' => ($row['period_amount'] == null OR $row['period_amount'] == 0) ? 'N/A' : $row['period_amount'],
            'loan_status' => $row['loan_atm_status'],
            'loan_status_desc' => 'อนุมัติ',
            'pay_type' => $row['pay_type'],
            'pay_per_month' =>  ($row['pay_per_month'] == 0  ? number_format($row["money_per_period"] , 2 , "." , ",") : $row['pay_per_month'])   ,
            'loan_type' => $row['loan_type']
        ];
...
$sql = "SELECT 
tb1.transaction_datetime,
tb1.loan_atm_id,tb4.contract_number, 
FORMAT(tb1.loan_amount_balance, 2) AS loan_balance,
IF(tb1.receipt_id, 'ชำระเงิน', 'ถอนเงิน ATM') AS trans_desc,
IF(tb1.receipt_id, 1, 0) AS trans_type,
FORMAT(tb2.loan_amount, 2) AS withdraw,
